...
def compile_classVarDec():   
    ...
    if kind == 'static':
        index_static += 1
        class_symbol_table[var_name] = {'type':var_type,'kind':kind,'index':index_static}
    elif kind == 'field':
        index_field += 1
        class_symbol_table[var_name] = {'type':var_type,'kind':kind,'index':index_field}
    # (',',varName)*
    while peek_next_token()[1] == ',':
        # ','
        append_this_token()
...
def compile_subroutineDec(): 
    ...
    append_this_token()
    if current_token[1] == 'method':
        subroutine_symbol_table['this'] = {'type':token_list[0][1],'kind':'argument','index':index_arg}
        index_arg += 1
    # 'void'|type
...
def compile_parameterList():
    ...
    while peek_next_token()[1] != ')':
        ...
        compile_varName()
        var_name = current_token[1]
        subroutine_symbol_table[var_name] = {'type':var_type,'kind':'argument','index':index_arg}
        while peek_next_token()[1] == ',':
            # ','
            append_this_token()
            compile_type()
...
def compile_varDec():
    ...
    compile_varName()
    var_name = current_token[1]
    subroutine_symbol_table[var_name] = {'type':var_type,'kind':'local','index':index_local}
    while peek_next_token()[1] != ';':
        # ','
        append_this_token()
        compile_varName()
