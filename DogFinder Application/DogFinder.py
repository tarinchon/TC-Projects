...
def ask_user_to_save(df):
    ...
    if choice == 'y':
        ...
        # Create new Pandas dataframe that stores saved search results
...
def main():
    """Retrieve access token on behalf of user. Then, aggregate and send in user's search criteria to PetFinder API, along with
    access token. Subsequently receive relevant data from PetFinder servers and invoke parse_and_print_data function to parse
    and print search results for user"""
    ...
    # Send get request to PetFinder servers along with user's access token 
